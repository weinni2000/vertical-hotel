...
class FolioRoomLine(models.Model):
    ...
    room_id = fields.Many2one("vertical_hotel.room", ondelete="restrict", index=True)
    check_in = fields.Datetime("Check In Date", required=True)
    check_out = fields.Datetime("Check Out Date", required=True)
    folio_id = fields.Many2one(
        "vertical_hotel.folio", "Folio Number", ondelete="cascade"
    )
    status = fields.Selection(related="folio_id.state", string="state")


class HotelFolio(models.Model):
    _name = "vertical_hotel.folio"
    ...
    room_line_ids = fields.One2many(
        "vertical_hotel.folio.line",
        "folio_id",
        help="Hotel room reservation detail.",
    )
    service_line_ids = fields.One2many(
        "vertical_hotel.service.line",
        "folio_id",
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            vals.setdefault("name", sequence.next_by_code("vertical_hotel.folio"))

...
class HotelServices(models.Model):
    ...
    def create(self, vals_list):
        service_type_obj = self.env["vertical_hotel.service.type"]
    ...
    def write(self, vals):
        ...
        """
        if "service_categ_id" in vals:
            service_categ_id = self.env["vertical_hotel.service.type"].browse(
...
class HotelServiceType(models.Model):
    _name = "vertical_hotel.service.type"
    _description = "Service Type"
    _inherits = {"product.category": "product_categ_id"}
    _rec_name = "name"

    service_id = fields.Many2one("vertical_hotel.service.type", "Service Category")
    child_ids = fields.One2many(
        "vertical_hotel.service.type", "service_id", "Service Child Categories"
    )
    ...
    def write(self, vals):
        if "service_id" in vals:
            service_categ = self.env["vertical_hotel.service.type"].browse(
                vals.get("service_id")
            )
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super().write(vals)

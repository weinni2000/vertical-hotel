...
class HotelRoom(models.Model):
    ...
    room_categ_id = fields.Many2one(
        "vertical_hotel.room.type", "Room Category", required=True, ondelete="restrict"
    )
    room_amenities_ids = fields.Many2many(
        "vertical_hotel.room.amenities",
    ...
    def create(self, vals_list):
        for vals in vals_list:
            room_type_obj = self.env["vertical_hotel.room.type"]
    ...
    def write(self, vals):
        ...
        """
        if "room_categ_id" in vals:
            room_categ = self.env["vertical_hotel.room.type"].browse(
...
class HotelRoomType(models.Model):
    _name = "vertical_hotel.room.type"
    _description = "Room Type"
    _inherits = {"product.category": "product_categ_id"}
    _rec_name = "name"

    categ_id = fields.Many2one("vertical_hotel.room.type", "Category")
    child_ids = fields.One2many(
        "vertical_hotel.room.type", "categ_id", "Room Child Categories"
    )
...
class HotelRoomAmenitiesType(models.Model):
    _name = "vertical_hotel.room.amenities.type"
    _description = "amenities Type"
    _inherits = {"product.category": "product_categ_id"}
    _order = "name"

    amenity_id = fields.Many2one("vertical_hotel.room.amenities.type", "Category")
    child_ids = fields.One2many(
        "vertical_hotel.room.amenities.type", "amenity_id", "Amenities Child Categories"
    )
    product_categ_id = fields.Many2one(
        "product.category",
        "Product Category",
    ...
    @api.model_create_multi
    def create(self, vals_list):
        rm_amenity_obj = self.env["vertical_hotel.room.amenities.type"]
        for vals in vals_list:
    ...
    def write(self, vals):
        if "amenity_id" in vals:
            amenity_categ = self.env["vertical_hotel.room.amenities.type"].browse(
                vals.get("amenity_id")
            )
            vals.update({"parent_id": amenity_categ.product_categ_id.id})
        return super().write(vals)
...
class HotelRoomAmenities(models.Model):
    _name = "vertical_hotel.room.amenities"
    ...
    amenities_categ_id = fields.Many2one(
        "vertical_hotel.room.amenities.type",
        "Amenities Category",
        required=True,
        ondelete="restrict",
    )
    ...
    def write(self, vals):
        ...
        """
        if "amenities_categ_id" in vals:
            amenities_categ = self.env["vertical_hotel.room.amenities.type"].browse(
